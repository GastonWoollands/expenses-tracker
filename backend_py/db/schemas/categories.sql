...
-- Categories Table
-- =============================================
-- Expense categories with hierarchical support and extensibility
-- Replaces the hardcoded categories with a flexible database structure

CREATE TABLE categories (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    key VARCHAR(50) UNIQUE NOT NULL, -- Unique key for API references
...
-- Insert all categories (65 total)
...
-- Comments
COMMENT ON TABLE categories IS 'Expense categories with hierarchical support';
COMMENT ON COLUMN categories.key IS 'Unique key for API references and programmatic access';
COMMENT ON COLUMN categories.name IS 'Display name for the category';
COMMENT ON COLUMN categories.description IS 'Detailed description of what belongs in this category';
...
COMMENT ON COLUMN categories.is_active IS 'Whether category is currently active';
COMMENT ON COLUMN categories.sort_order IS 'Sort order for display purposes';
