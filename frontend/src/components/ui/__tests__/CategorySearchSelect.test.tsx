...
describe('CategorySearchSelect', () => {
  ...
  it('renders with placeholder text', () => {
    render(<CategorySearchSelect value="" onChange={mockOnChange} placeholder="Select category" />);
  ...
  it('filters categories based on search term', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/groceries, dining out, coffee, alcohol/i)).toBeInTheDocument();
    });
  });
  ...
  it('displays selected category', () => {
    render(<CategorySearchSelect value="food" onChange={mockOnChange} placeholder="Select category" />);
  ...
  it('shows search results count', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/\d+ of \d+ categories/)).toBeInTheDocument();
    });
  });

  it('applies required styling when required and no value', () => {
    render(<CategorySearchSelect value="" onChange={mockOnChange} placeholder="Select category" required />);

    const trigger = screen.getByRole('button');
    expect(trigger.className).toMatch(/border-red-/);
  });
});
